...
pub fn tool_definitions() -> Vec<ToolDefinition> {
    vec![
        ...
        ToolDefinition {
            ...
            description: "Create an editable mask tied to a source image. Writes three files: the `.mask` state (ASON), a greyscale mask PNG at source dimensions (feed this as `mask` to `inpaint_image`), and a 512x512 annotated preview JPEG showing an 8x8 grid with column letters A-H and row numbers 1-8 on all four sides, with masked cells tinted red. Use `fill_slot` afterwards to toggle individual cells or rectangles. Red = will be regenerated by inpainting, unmarked = preserved. By default creates an empty mask (`init: \"none\"`); other init modes are available for quick experimentation.".into(),
            input_schema: serde_json::json!({
                "type": "object",
                "properties": {
                    "source_image": {
                    ...
                    "init": {
                        "type": "string",
                        "enum": ["none", "random", "all", "checkerboard"],
                        "description": "Initial cell values. 'none' (default) starts with all cells off; use `fill_slot` to add cells. 'random' / 'checkerboard' / 'all' are quick starters for experimentation.",
                        "default": "none"
                    },
                    "seed": {
                        "type": "integer",
        ...
        ToolDefinition {
            name: "fill_slot".into(),
            description: "Fill (or clear) cells in an existing mask created by `create_mask`. The `cells` string accepts: single cells like \"A2\", inclusive rectangles like \"A2-C4\" (any corner order - \"C4-A2\" is the same), and comma-separated combinations like \"A2, B3-D5, E1\". Letter/digit order inside a cell is flexible (\"1A\" and \"A1\" are identical); whitespace is ignored; cells are case-insensitive. `with` is 1 (fill - that region will be replaced by inpainting) or 0 (clear - preserved); default 1. The .mask state and both sidecars are regenerated on every call; the response returns all three paths plus the updated preview inline. Use iteratively to shape a mask, then pass `final_mask_path` to `inpaint_image`.".into(),
            input_schema: serde_json::json!({
                "type": "object",
                "properties": {
                    "path": {
                        "type": "string",
                        "description": "Absolute path to an existing `.mask` file. `.mask` suffix is appended if missing."
                    },
                    "cells": {
                        "type": "string",
                        "description": "Cell-spec string. Examples: 'A2', 'A2, B3', 'A2-C4', 'A2-C4, E1-E3, H8', 'a1' (case-insensitive), '1A' (order-insensitive)."
                    },
                    "with": {
                        "type": "integer",
                        "enum": [0, 1],
                        "description": "0 = clear (cell preserved by inpaint), 1 = fill (cell replaced). Default 1.",
                        "default": 1
                    }
                },
                "required": ["path", "cells"]
...
pub async fn handle_tool_call(
    ...
    match tool_name {
        ...
        "fill_slot" => crate::mask::handle_fill_slot(args).await,
